...
Note that there may be multiple subarrays that add up to the target sum, but your function only needs to return the indices of any one such subarray. 
...
def subarray_sum(nums, target):
    ...
    for i, num in enumerate(nums):
        ...
        if compliment in sum_index:
            starting_index = sum_index[compliment] + 1
